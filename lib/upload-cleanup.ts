...
const uploadRoot = () => process.env.UPLOAD_DIR?.trim() || "data/uploads";
const safeName = (name: string) => /^[a-zA-Z0-9.-]+$/.test(name);
const filePath = (name: string) => process.env.UPLOAD_DIR?.trim() ? path.join(/*turbopackIgnore: true*/ process.env.UPLOAD_DIR.trim(), name) : path.join("data/uploads", name);
