...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawTickets = Array.isArray(body.uploadTickets) ? body.uploadTickets : body.uploadTicket ? [body.uploadTicket] : [];
    if (!rawTickets.length) return NextResponse.json({ error: "上传凭证已失效，请重新选择图片" }, { status: 400 });
    const ticketNameLists = await Promise.all(rawTickets.map((ticket: unknown) => verifyUploadTicket(String(ticket), auth.userId)));
    const ticketNames = [...new Set(ticketNameLists.flat())];
