...
import { getArtworkDisplays, getArtworkOriginals } from "@/lib/image";
...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const images = getArtworkDisplays(data.artwork);
  const originals = getArtworkOriginals(data.artwork);
      ...
      <main className="mx-auto max-w-7xl px-5 py-8">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_350px]">
          ...
          <aside>
            ...
            {originals.length>0 && <div className="mt-5 flex flex-wrap gap-2">{originals.map((url,index)=><a key={url} href={url} className="btn inline-block" download>下载原图{originals.length>1?` ${index+1}`:""}</a>)}</div>}
            ...
            {!data.isOwner&&<div className="mt-5"><ReportButton type="artwork" id={data.artwork.id} /></div>}
          </aside>
        </div>
      </main>
      <Footer />
