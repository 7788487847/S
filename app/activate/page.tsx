...
import { activateAccount } from "@/lib/client-accounts";
...
function Inner() {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/activate", { method: "POST", cache: "no-store", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: activationToken }), signal: controller.signal }).then(async response => {
      ...
      activateAccount({ token: result.token, user: (result.user || {}) as { id?:number; displayName?:string; username?:string; role?:"artist"|"seeker" } });
      setOk(true);
