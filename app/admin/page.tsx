  ...
  const load = useCallback(async (currentToken: string) => {
    const results = await Promise.all(tabs.filter(([key])=>key!=="site").map(async ([key, name]) => {
      try {
        ...
        if (response.status === 401 || response.status === 403) return { key, name, forbidden: true, ok: false, items: [] as Item[] };
        if (!response.ok) return { key, name, forbidden: false, ok: false, items: [] as Item[] };
        return { key, name, forbidden: false, ok: true, items: await response.json() as Item[] };
      } catch { return { key, name, forbidden: false, ok: false, items: [] as Item[] }; }
    }));
    ...
    setNotice(failed.length ? `${failed.join("、")}数据加载失败，其余模块仍可使用` : "");
  }, []);
