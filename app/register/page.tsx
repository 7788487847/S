...
async function submit(event: React.FormEvent) {
  ...
  try {
    ...
    const result: { message?: string; error?: string; activationUrl?: string } =
    ...
    <main className="grid min-h-[75vh] place-items-center p-5">
      ...
      >
        ...
        <label className="flex gap-2 text-sm">
          <input
            type="checkbox"
            checked={form.age14Confirm}
            onChange={(e) => update({ age14Confirm: e.target.checked })}
          />
          我已满14周岁
        </label>
        <label className="flex gap-2 text-sm">
          <input
            type="checkbox"
            checked={form.adultConfirm}
            onChange={(e) => update({ adultConfirm: e.target.checked })}
          />
          我已阅读并同意用户协议和隐私政策
        </label>
